...
object HangulInitialSoundSearch {
    ...
    private fun getInitialSound(char: Char): Char = initialSounds[(char.code - HANGUL_BEGIN_UNICODE) / HANGUL_BASE_UNIT]
        ...
        for (i in 0..seof) {
            ...
            while (t < slen) {
                if (isInitialSound(trimSearch[t]) && isHangul(trimValue[i + t])) {
                    if (getInitialSound(trimValue[i + t]) == trimSearch[t]) {
                        t++
                    } else {
                        break
                    }
                } else {
                    if (trimValue[i + t] == trimSearch[t]) {
                        t++
                    } else {
                        break
                    }
