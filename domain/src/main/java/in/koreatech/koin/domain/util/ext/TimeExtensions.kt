...
val localDayOfWeekName get() =
    when (localDateTimeNow.dayOfWeek.value) {
        1 -> "MONDAY"
        2 -> "TUESDAY"
        3 -> "WEDNESDAY"
        4 -> "THURSDAY"
...
fun LocalTime.isEqualOrBigger(time: LocalTime): Boolean = this.isAfter(time) || this.equals(time)

// this <= time (17:00 <= 22:00)
fun LocalTime.isEqualOrSmaller(time: LocalTime): Boolean = this.isBefore(time) || this.equals(time)

fun LocalTime.isAm(): Boolean = this.isBefore(LocalTime.of(12, 0))

fun LocalTime.isPm(): Boolean = this.isAfter(LocalTime.of(12, 0)) || this.equals(LocalTime.of(12, 0))
