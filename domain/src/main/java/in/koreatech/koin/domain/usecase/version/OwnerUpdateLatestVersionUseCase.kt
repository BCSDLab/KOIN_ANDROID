...
class OwnerUpdateLatestVersionUseCase @Inject constructor(
    ...
    suspend operator fun invoke(versionCode: Int) =
        runCatching {
            if (versionCode > 99999 || versionCode < 10000) {
                throw IllegalArgumentException(
                    "VersionCode must be in 10000 to 99999| input: $versionCode"
                )
            }
            ...
            versionRepository.getLatestVersionCode()?.let { latestVersionCode ->
                if (versionCode < latestVersionCode) {
                    throw IllegalArgumentException(
                        "VersionCode must be higher than before VersionCode| input: $versionCode, origin: $latestVersionCode"
                    )
                }
            }
