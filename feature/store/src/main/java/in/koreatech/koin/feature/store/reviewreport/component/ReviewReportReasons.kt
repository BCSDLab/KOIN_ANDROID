...
import `in`.koreatech.koin.feature.store.REPORT_OTHER_REASON_MAX_LENGTH
import `in`.koreatech.koin.feature.store.reviewreport.enums.ReportReason
import kotlinx.collections.immutable.ImmutableList
import kotlinx.collections.immutable.persistentListOf

@Composable
fun ReviewReportReasons(
    itemList: ImmutableList<ReportReason>,
        ...
        itemList.forEachIndexed { index, reportReason ->
            if (reportReason == ReportReason.OTHER) {
                LostAndFoundReportReasonOtherItem(
                    onFocused = {
                        onSelectedItemChange(reportReason)
                    },
                    isSelected = selectedItem.contains(reportReason),
                    reason = otherReason,
    ...
    Row(
        ...
        Image(
            modifier = Modifier
                ...
                .size(16.dp),
            imageVector = ImageVector.vectorResource(id = if (isSelected) R.drawable.ic_review_report_selected else R.drawable.ic_review_report_unselected),
...
fun LostAndFoundReportReasonOtherItem(
    ...
    Column(
        ...
        ReportTextField(
            ...
            placeholder = stringResource(id = R.string.review_report_other_reason),
            onFocused = {
                if (!isSelected) {
                    onFocused()
                }
