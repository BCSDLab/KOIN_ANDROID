...
@Composable
fun FindPasswordByEmail(
    viewModel: FindPasswordByEmailViewModel = hiltViewModel(),
...
@Composable
fun FindPasswordByEmailImpl(
    email: String,
    emailState: PhoneNumber,
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.find_password_email),
        ...
        when (emailState) {
            PhoneNumber.CountExceeded -> EmailRequestCountExceeded()
            is PhoneNumber.Sent -> VerificationCodeSentSuccessMessage(
                emailState.remainingCount,
                emailState.totalCount
            )

            PhoneNumber.WrongFormat -> EmailInvalidMessage()
...
@Composable
private fun EmailInvalidMessage() {
    Spacer(modifier = Modifier.height(8.dp))

    KoinFindPasswordTextFieldAlert(
        text = stringResource(R.string.find_password_email_invalid),
        state = KoinFindPasswordTextFieldAlertState.Warning
    )
}

@Composable
private fun EmailNotMatch() {
    Spacer(modifier = Modifier.height(8.dp))
...
@Composable
private fun EmailRequestCountExceeded() {
    Spacer(modifier = Modifier.height(8.dp))

    KoinFindPasswordTextFieldAlert(
        text = stringResource(R.string.find_password_email_code_request_exceeded),
...
fun handleSideEffect(
    sideEffect: FindPasswordByEmailSideEffect,
    onStartTimer: () -> Unit = {},
    onStopTimer: () -> Unit = { }
) {
    when (sideEffect) {
        FindPasswordByEmailSideEffect.StartTimer -> onStartTimer()
        FindPasswordByEmailSideEffect.StopTimer -> onStopTimer()
    }
}

@Preview(showBackground = true)
@Composable
fun FindPasswordByEmailPreview() {
    FindPasswordByEmailImpl(
        email = "test@test.com",
