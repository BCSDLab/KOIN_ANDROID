...
class StoreDetailReviewRecyclerAdapter(
        ...
        with(holder) {
            ...
            updateAt.text =
                if (review.isModified) {
                    String.format(ContextCompat.getString(itemView.context, R.string.store_review_is_modify), review.createdAt.replace("-", "."))
                } else {
                    review.createdAt.replace("-", ".")
                }
    ...
    inline fun setOnItemClickListener(crossinline onItemClick: (Id: Int) -> Unit) {
        onItemClickListener =
            object : StoreDetailReviewRecyclerAdapter.OnItemClickListener {
