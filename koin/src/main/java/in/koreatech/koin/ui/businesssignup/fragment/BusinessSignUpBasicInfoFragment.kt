...
class BusinessSignUpBasicInfoFragment : BaseFragment() {
    ...
    private fun inputTextEffect() =
        with(binding) {
            signupEdittextId.doOnTextChanged { text, _, _, _ ->
                if (text.isNullOrBlank()) {
                    divideLine1.setBackgroundColor(this@BusinessSignUpBasicInfoFragment.requireContext().getColor(R.color.blue1))
                } else {
                    divideLine1.setBackgroundColor(this@BusinessSignUpBasicInfoFragment.requireContext().getColor(R.color.black))
                }
                isAllWrite()
            }
            ...
            signupEdittextPw.doOnTextChanged { text, _, _, _ ->
                if (text.isNullOrBlank()) {
                    divideLine2.setBackgroundColor(this@BusinessSignUpBasicInfoFragment.requireContext().getColor(R.color.blue1))
                } else {
                    divideLine2.setBackgroundColor(this@BusinessSignUpBasicInfoFragment.requireContext().getColor(R.color.black))
                }
                isAllWrite()
            }
            ...
            signupEdittextPwConfirm.doOnTextChanged { text, _, _, _ ->
                if (text.isNullOrBlank()) {
                    divideLine3.setBackgroundColor(this@BusinessSignUpBasicInfoFragment.requireContext().getColor(R.color.blue1))
                } else {
                    divideLine3.setBackgroundColor(this@BusinessSignUpBasicInfoFragment.requireContext().getColor(R.color.black))
                }
                isAllWrite()
            }
        }
    ...
    private fun isAllWrite() {
        ...
        if (check) {
            binding.signupSendVerificationButton.setBackgroundColor(this.requireContext().getColor(R.color.colorPrimary))
        } else {
            binding.signupSendVerificationButton.setBackgroundColor(this.requireContext().getColor(R.color.gray5))
        }
    }
