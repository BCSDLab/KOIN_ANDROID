...
public class FreeBoardCommentTest {
    ...
    @Rule
    public ActivityTestRule<BoardActivity> activityRule =
            new ActivityTestRule<>(BoardActivity.class);
    /**
     * 앱바 이름 확인
     */
    @Test
    ...
    public void testcaseForCancelButtonClick(){
        onView(withId(R.id.freeboard_recyclerview))
                .perform(RecyclerViewActions.actionOnItemAtPosition(0,click()));    //첫번째 게시글 클릭
    ...
    public void testCaseForArticleCommentEdit(){
        ...
        onView(withId(R.id.article_comment_register_button)).perform(click());                          //등록 버튼 클릭
        onView(withText("내용을 입력해주세요.")).inRoot(withMessage("내용을 입력해주세요."))         //토스트 메시지 도출
        ...
        onView(withId(R.id.article_comment_register_button)).perform(click());                             //등록 버튼 클릭
        onView(withText("댓글이 수정되었습니다.")).inRoot(withMessage("댓글이 수정되었습니다."))       //토스트메시지 도출
                .check(matches(isDisplayed()));
    ...
    public void testcaseForDeleteComment(){
        ...
        onView(withText("댓글이 등록되었습니다.")).inRoot(withMessage("댓글이 등록되었습니다."))               //토스트 메시지 확인
                .check(matches(isDisplayed()));
        ...
        onView(withText("댓글이 삭제되었습니다.")).inRoot(withMessage("댓글이 삭제되었습니다."))     //토스트 메시지 도출
                .check(matches(isDisplayed()));
    }
